...
public:
    ...
    typedef enum {ONE_MOVE, TWO_SLIDE, TWO_MOVE, TWO_PUSH_ONE, TWO_KILL_ONE, THREE_SLIDE, THREE_MOVE, THREE_PUSH_ONE, THREE_KILL_ONE, THREE_PUSH_TWO, THREE_PUSH_ONE_KILL_ONE, NO_MOVE} MoveType;
    typedef struct {int f_m; int l_m; Direction dir; MoveType mt;} move_record_t;
    /*
     *    0 \   / 1
     *  5 <-     -> 2
     *    4 /   \ 3
    ...
    virtual MoveCode makeMove(MarbleColor color, int first_marble_pos, int last_marble_pos, Direction d, MoveType& mt, bool activate) = 0;
    virtual MoveCode testMove(MarbleColor color, int first_marble_pos, int last_marble_pos, Direction d, MoveType& mt) const = 0;
